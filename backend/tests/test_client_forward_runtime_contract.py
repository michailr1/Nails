...
def test_permanent_deploy_owns_client_runtime_lifecycle():
    ...
    assert "CLIENT_API_ENABLED and CLIENT_BOT_ENABLED must be enabled together" in deploy
    assert "client and trusted master Telegram bot tokens must differ" in deploy
    assert 'compose build --build-arg GIT_SHA="$RELEASE_SHA" nails-api nails-web nails-client-bot' in deploy
    ...
    assert 'compose up -d --no-deps --force-recreate --no-build nails-client-bot' in deploy
