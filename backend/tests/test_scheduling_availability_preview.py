...
def test_preview_positive_window_change_does_not_displace_booking(
    client,
    create_user,
    create_service,
    create_availability,
    ...
    with get_session_factory()() as session:
        ...
        session.add(
            Booking(
                ...
                idempotency_key="preview-positive-window-booking",
            )
        )
        session.commit()
...
def test_preview_day_off_reports_existing_booking_conflict(
    client,
    create_user,
    create_service,
    create_availability,
    ...
    with get_session_factory()() as session:
        ...
        session.add(
            Booking(
                ...
                currency="RUB",
                price_source="service_snapshot",
                idempotency_key="preview-day-off-booking",
            )
        )
        session.commit()
