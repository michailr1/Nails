...
def test_manicure_coating_does_not_double_count_removal_and_reinforcement(
    ...
    addons = {item["public_name"]: item for item in booking["catalog_items"] if item["kind"] == "addon"}
...
def test_repair_three_nails_scales_time_price_and_snapshot(
    ...
    repair = next(item for item in booking["catalog_items"] if item["public_name"] == "Ремонт ногтя")
