...
## 4. Открытые PR (проверь актуальный CI перед действиями)

- **#63** `docs/existing-mechanisms-check` — правило «перечисли существующие механизмы до проектирования» + ревью границ абстракций. **Смержить первым**: прямо предотвращает грабли §6. База устарела — rebase на main.
- **#78** `docs/lint-autofix-rule` — правило «CI-lint чинится только `ruff --fix`, руками импорты не трогать». Процессное, смержить рано.
- **#80** `docs/adr-004-client-contour` — ADR-004: клиентский контур v1 = детерминированный бот без LLM (кнопки + заявки с подтверждением мастером, роль `client`, отдельная личность). Согласовано владельцем.
- **#81** `fix/client-card-guidance` — расширенные поля карточки клиентки. **Не готов, см. §5.**
...
```text
1. rebase и merge процессных правил #63 и #78 (предотвращают грабли §6)
2. вернуть PR #81 автору: удалить мёртвый operations._create_client, свести тесты к client_cards, зелёный pytest
3. merge #80 (ADR-004)
4. по готовности PR — candidate-validation по exact PR-head SHA, затем ff-merge того же SHA (ADR-003)
5. затем NAILS-002F: бэкапы + проверенный restore
```
