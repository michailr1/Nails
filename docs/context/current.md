...
Issue #125 — ADR-007 Slice B: booking catalog snapshots.
...
Slice B — обязательный промежуточный release для смены rollback baseline:
...
5. только после deployment и закрытия rollback-окна следующий slice активирует допы, range, per-unit, on-request и overrides.
...
## Сознательно не входит в Slice B
...
```text
1. завершить реализацию и tests Slice B
2. открыть draft PR к main
3. review exact diff и rollback-контракт
4. получить зелёный CI
5. candidate deployment exact PR head
6. fast-forward main тем же SHA
7. finalize production
8. начать Slice C activation + overrides
```
