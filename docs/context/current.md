...
2. определить owner-scoped pending client identity отдельно от операторских `users`;
3. запретить автоматическую привязку существующей карточки по имени, username или телефону;
4. определить заявку как отдельную сущность, не резервирующую время;
5. разбить реализацию на backend foundation, кабинет мастера, deterministic bot и управление своими заявками;
6. не начинать runtime client bot до принятия backend security contract.

## Следующий приоритет

ADR-004 Slice A — backend foundation:

- owner-scoped `client_telegram_identity` со статусами `pending/active/revoked` и nullable `client_id` до решения мастера;
- явное master resolution: связать с существующей owner-scoped карточкой либо создать новую;
...
- client identity/create/list/cancel собственных заявок;
- master list/resolve/approve/reject через существующий `create_booking`;
- recoverable approve после внутреннего Booking commit через детерминированный idempotency key;
- isolation, privacy, idempotency, audit, crash-gap и overlap regression tests;
...
client_identity_boundary=pending until explicit master resolution; no auto-link by name or phone
booking_request_boundary=pending does not reserve; approve reuses idempotent create_booking
