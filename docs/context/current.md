...
- owner-scoped `POST /web/api/clients` поверх существующего `create_or_reuse_client`;
- обязательные session и Origin/Host boundary;
- idempotency key и server-side повторная проверка записи;
- выбор существующей клиентки либо компактное подтверждаемое `+ Новая клиентка` с обязательным именем и необязательным телефоном;
- созданная или переиспользованная клиентка сразу выбирается в текущей записи;
- выбор основной процедуры и нескольких дополнений;
...
- при пересечении возвращаются все конфликтующие записи без внутренних ID: клиентка, процедура, дополнения, фактическое время и занятый интервал с учётом времени до/после;
- web показывает эти сведения мастеру вместо общего сообщения «пересекается с другой записью»;
- browser получает только web-представления клиентки и записи, без внутренних ID состава каталога;
- миграций и изменения доменной booking-модели нет.

Полноценное редактирование клиентки намеренно не встраивается в форму записи: там остаётся быстрый путь «имя + телефон», а карточка с заметками и предпочтениями будет отдельным web-slice.

## Следующий приоритет

1. Завершить `feat/web-booking-composer`: новый green CI → exact-head candidate → ручная проверка быстрого добавления клиентки и полного сообщения о пересечении → merge → production deploy.
2. Реализовать редактирование клиенток в разделе «Клиентки»: список → карточка → имя, телефон, заметки, состояние ногтей/кожи, чувствительность, стиль и предпочтения по общению.
3. Перенести реальный прайс мастера через существующий подтверждаемый batch replace, разделяя варианты через `/` на отдельные позиции.
4. После живой работы уточнить только реальные пробелы: количество для цены за единицу и применимость дополнений.
...
next=finish PR #167 after full booking overlap details, then start web client card editing
```
