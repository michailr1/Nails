...
### 5.4a Выбор состава клиенткой и заметка мастеру

Решение владельца (2026-08-02) по итогам первого показа клиентского бота: плоский список из ~20 позиций каталога нечитаем — клиентка теряется. Каталог мастера отражает её учёт, а не намерение клиентки.

- **Выбор идёт по разделам** (`category`, уже есть в каталоге): раздел → 4–6 позиций раздела. Плоский список всех позиций не показывается.
- **«Как в прошлый раз»** — первым действием для вернувшейся клиентки: состав прошлого визита одним нажатием. В этом бизнесе большинство визитов повторяется, и для постоянной клиентки правильный интерфейс — не список, а подтверждение.
- **Допы — опциональны**: клиентка может выбрать только базовую процедуру.
- **Заметка мастеру** — необязательное свободнотекстовое поле на шаге сводки, одно на заявку. Мастер видит его в карточке заявки и в уведомлении о новой заявке.

**Инвариант: свободный текст никогда не меняет состав, время и цену.** Заметка — информация для мастера, а не команда системе; интерпретации недоверенного ввода в доменные аргументы не происходит (усиливает §3.4). Состав и время меняет мастер явным действием при подтверждении.

Требования к заметке: экранирование при рендере в Telegram-сообщение мастеру (иначе разметка из недоверенного источника попадает в её чат), ограничение длины, подавление превью ссылок, отсутствие в audit (§6) и в уведомлениях обратно клиентке.

Если мастер при подтверждении меняет состав и запись перестаёт помещаться, пересечение показывается ей **до** сохранения — существующая overlap-проверка не прячется в этом сценарии.
