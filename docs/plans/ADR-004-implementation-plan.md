...
**Решение владельца (2026-07-28): никакого подтверждения мастера до первой заявки.** Клиентка, перешедшая по ссылке мастера, сразу видит прайс и отправляет заявку — отдельный шаг «мастер разрешил клиентку» перед этим запрещён как продуктовый гейт (тот же §3.11: открыто по умолчанию). Следствия, обязательные к соблюдению:

- `booking_requests.client_id` остаётся **nullable** до решения первой заявки; требование NOT NULL закрывает продуктовый сценарий и не допускается;
- новая Telegram-identity в статусе `pending` — полноценное основание смотреть публичный прайс, свободные окна и создать заявку;
- разрешение карточки выполняется мастером **внутри** решения первой заявки, а не отдельным предварительным шагом;
- инвариант §3.2 не ослабляется: отсутствие гейта **не означает** автоматическую привязку к существующей карточке по совпадению имени или телефона. Не разрешённая заявка несёт заявленное публичное имя; при approve мастер явно выбирает «связать с существующей карточкой» или «создать новую»;
- подтверждение мастером самой **записи** сохраняется без изменений: заявка не становится записью автоматически (ADR-004 §3 «Решение», п.3).
