...
class RetentionTest(unittest.TestCase):
    ...
    def test_daily_weekly_monthly_limits_and_same_day_deduplication(self) -> None:
        for day in range(7):
            ...
            self.touch(self.root / "daily" / f"nails-{stamp}T010000Z.sql.gz", timedelta(days=day, hours=2))
            self.touch(self.root / "daily" / f"nails-{stamp}T020000Z.sql.gz", timedelta(days=day, hours=1))
        for index in range(5):
            self.touch(self.root / "weekly" / f"nails-2026-W{index:02d}.sql.gz", timedelta(weeks=index))
        for index in range(14):
            self.touch(self.root / "monthly" / f"nails-2025{index + 1:02d}.sql.gz", timedelta(days=index * 30))
    ...
    def test_predeploy_keeps_all_24h_then_one_per_day_for_five_days(self) -> None:
        recent_a = self.touch(self.root / "nails-before-deploy-a.sql.gz", timedelta(hours=2))
        recent_b = self.touch(self.root / "nails-before-deploy-b.sql.gz", timedelta(hours=20))
        old_same_day_a = self.touch(self.root / "nails-before-deploy-c.sql.gz", timedelta(days=2, hours=3))
        old_same_day_b = self.touch(self.root / "nails-before-deploy-d.sql.gz", timedelta(days=2, hours=1))
        expired = self.touch(self.root / "nails-before-deploy-e.sql.gz", timedelta(days=6))
    ...
    def test_runtime_keeps_two_successes_failed_three_days_and_local_patches(self) -> None:
        for index in range(4):
            self.touch(self.runtime / f"deploy-success-{index}", timedelta(hours=index + 1))
        failed_recent = self.touch(self.runtime / "deploy-failed-recent", timedelta(days=2))
        failed_old = self.touch(self.runtime / "deploy-failed-old", timedelta(days=4))
        patches = self.touch(self.runtime / "hermes-local-patches", timedelta(days=100))
