...
NAILS_SCHEDULING = {
    ...
    "description": (
        ...
        "requested or supplied in arguments. Resolve absolute, relative, and weekday-based "
        "dates through the backend before using them; never calculate calendar dates or "
        "weekdays in the model. Read services, a concrete calendar day, free slots, or an "
        "exact client match. Replace availability only for explicitly named dates after a "
        "before/after summary and explicit confirmation; unrelated dates are preserved and "
        "existing scheduled bookings are protected. Create a client only after an exact "
        "lookup, a typo check, and explicit user confirmation. Create a booking only after "
        "the client and service are resolved, the chosen time was returned by free_slots, "
        "a final human-readable summary was shown, and the user explicitly confirmed it. "
        "Use only weekday_iso returned by the backend. The runtime generates booking "
        "idempotency data and copies commercial and timing snapshots from backend service data."
    ),
    "parameters": {
        "type": "object",
        "additionalProperties": False,
        "properties": {
            ...
            "date_kind": {
                "type": "string",
                "enum": ["absolute", "relative_days", "weekday"],
            ...
            "occurrence": {
                "type": "string",
                "enum": ["nearest_future", "current_week", "next_week"],
                "description": "How resolve_date selects a weekday occurrence.",
            },
            "days": {
                "type": "array",
